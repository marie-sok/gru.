...
public class MessageService {
    ...
    public Message send(String senderId, SendMessageRequest request) {
        ...
        ReplyReference replyTo = buildReplyReference(chat, request.getReplyToMessageId());

        List<String> receivers = chat.getParticipants().stream()
        ...
        if (receivers.isEmpty()) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Chat has no receiver");
        }
        if (receivers.size() > 1) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Group messages are not supported yet");
        }
    ...
    public List<Message> getMessages(String userId, String chatId) {
        requireUser(userId);
        Chat chat = getChat(chatId);
        ...
        return messageRepository.findByChatIdOrderByCreatedAtAsc(chatId).stream()
                .filter(message -> message.getHiddenForUserIds() == null
                        || !message.getHiddenForUserIds().contains(userId))
                .toList();
    }

    public Message markDelivered(String userId, String messageId) {
        requireUser(userId);
        Message message = getMessage(messageId);
        requireReceiver(message, userId);
    ...
    public Message markRead(String userId, String messageId) {
        requireUser(userId);
        Message message = getMessage(messageId);
        requireReceiver(message, userId);
    ...
    public List<Message> markChatRead(String userId, String chatId) {
        requireUser(userId);
        Chat chat = getChat(chatId);
        requireParticipant(chat, userId);
        ...
        for (Message message : messages) {
            if (message.getHiddenForUserIds() != null && message.getHiddenForUserIds().contains(userId)) {
                continue;
            }
            if (!userId.equals(message.getReceiverId())) continue;
            ...
            if (didChange) changed.add(messageRepository.save(message));
        }
        return changed;
    }
    ...
    public Message setReaction(String userId, String messageId, String reaction) {
        requireUser(userId);
        if (reaction == null || reaction.isBlank()) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Reaction is required");
        }
        Message message = getVisibleMessageForUser(userId, messageId);
        message.setReaction(reaction.trim());
        return messageRepository.save(message);
    }

    public Message removeReaction(String userId, String messageId) {
        requireUser(userId);
        Message message = getVisibleMessageForUser(userId, messageId);
        message.setReaction(null);
        return messageRepository.save(message);
    ...
    public Message deleteForMe(String userId, String messageId) {
        requireUser(userId);
        Message message = getMessage(messageId);
        ...
        if (message.getHiddenForUserIds() == null) {
            message.setHiddenForUserIds(new HashSet<>());
        }
        message.getHiddenForUserIds().add(userId);
        return messageRepository.save(message);
    }

    public Message deleteForEveryone(String userId, String messageId) {
        requireUser(userId);
        Message message = getMessage(messageId);
    ...
    public Message edit(String senderId, String messageId, String newText) {
        requireUser(senderId);
    ...
    private ReplyReference buildReplyReference(Chat chat, String replyToMessageId) {
        if (replyToMessageId == null || replyToMessageId.isBlank()) return null;
        Message original = getMessage(replyToMessageId.trim());
        if (original.getChatId() == null || !original.getChatId().equals(chat.getId())) {
            throw new ResponseStatusException(
                    HttpStatus.BAD_REQUEST,
                    "Reply message does not belong to this chat"
            );
        }
        return new ReplyReference(original.getId(), original.getSenderId(), original.getText());
    }
    ...
    private Message getVisibleMessageForUser(String userId, String messageId) {
        Message message = getMessage(messageId);
        Chat chat = getChat(message.getChatId());
        requireParticipant(chat, userId);
        if (message.getHiddenForUserIds() != null && message.getHiddenForUserIds().contains(userId)) {
    ...
    private void requireMessagingAllowed(String senderId, String receiverId) {
        ...
        boolean senderBlockedReceiver = sender.getBlockedUserIds() != null
                && sender.getBlockedUserIds().contains(receiverId);
        boolean receiverBlockedSender = receiver.getBlockedUserIds() != null
                && receiver.getBlockedUserIds().contains(senderId);
    ...
    private void requireParticipant(Chat chat, String userId) {
    ...
    private void requireReceiver(Message message, String userId) {
