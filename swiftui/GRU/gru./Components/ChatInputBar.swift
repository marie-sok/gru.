...
private enum GRURecordMode: Equatable {
    ...
    var title: String {
        switch self {
        case .voice: return GRUL10n.text("голосовое")
        case .videoNote: return GRUL10n.text("видео")
        }
    }
}
...
struct ChatInputBar: View {
    ...
    private var recordingUIVisible: Bool {
        touchActive || didStartRecordingGesture || isInlineVoiceRecording ||
        audioRecorder.isRecording || audioRecorder.isPreparing || recordingLocked
    }
    ...
    var body: some View {
        ...
        .onDisappear {
            ...
            if recordMode == .videoNote && (didStartRecordingGesture || recordingLocked) {
    ...
    private var attachmentButton: some View {
        ...
        .accessibilityLabel(GRUL10n.text(showMenu ? "Закрыть вложения" : "Добавить вложение"))
    }

    private var textField: some View {
        TextField(GRUL10n.text("Сообщение"), text: $text, axis: .vertical)
            .textFieldStyle(.plain)
            .submitLabel(sendByReturn ? .send : .return)
            .onSubmit {
                if sendByReturn && canSend { sendMessage() }
            }
            .lineLimit(1...6)
    ...
    private var recordingStrip: some View {
        HStack(spacing: 9) {
            ...
            if cancelRecording {
                Text(GRUL10n.text("Отпусти — отменим"))
                    .font(.system(size: 13, weight: .semibold, design: .rounded))
                    .foregroundStyle(.red)
                    .lineLimit(1)
                Spacer(minLength: 0)
            } else if recordingLocked {
                Text(GRUL10n.text("Запись зафиксирована"))
                    .font(.system(size: 13, weight: .semibold, design: .rounded))
                ...
                Text(GRUL10n.text("Отпусти для отправки"))
                    .font(.system(size: 13, weight: .semibold, design: .rounded))
                    .foregroundStyle(.secondary)
                    .lineLimit(1)
                Spacer(minLength: 0)
    ...
    private var lockedVoiceSendButton: some View {
        Button { finishLockedVoiceRecording() } label: {
            GRUNeonIcon(systemName: "paperplane.fill", size: 42, iconSize: 18)
        }
        .buttonStyle(.plain)
        .accessibilityLabel(GRUL10n.text("Отправить голосовое"))
    }
    ...
    private var recordModeHint: some View {
        HStack(spacing: 8) {
            Image(systemName: recordMode.badgeIcon)
                ...
                .foregroundStyle(GRUColors.accent)
            Text(GRUL10n.text("двойной тап — режим"))
                .font(.system(size: 9, weight: .semibold, design: .rounded))
                .foregroundStyle(.secondary)
            Circle().fill(Color.secondary.opacity(0.38)).frame(width: 3, height: 3)
            Text(GRUL10n.text("удержание — запись"))
                .font(.system(size: 9, weight: .semibold, design: .rounded))
                .foregroundStyle(.secondary)
            Spacer(minLength: 0)
        }
        ...
        .overlay { Capsule().stroke(GRUColors.accent.opacity(0.13), lineWidth: 1) }
    }
    ...
    private var recordingButton: some View {
        ...
        .accessibilityLabel(GRUL10n.text("Двойной тап переключает режим записи. Удерживай для записи. Свайп влево отменяет, вверх фиксирует."))
    ...
    private func updateRecordDrag(_ translation: CGSize) {
        ...
        if horizontalCancel {
            if !cancelRecording, hapticsEnabled {
