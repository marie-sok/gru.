...
struct BetaChatListView: View {
    ...
    private var compactConnectionNotice: some View {
        HStack(spacing: 7) {
            ...
            Text(service.isUsingCachedChats ? "offline • показываю сохранённые чаты" : "backend недоступен")
    ...
    private var emptyState: some View {
        VStack(spacing: 12) {
            ...
            Image(systemName: searchText.isEmpty ? "envelope.open" : "magnifyingglass")
                .font(.system(size: 28, weight: .medium))
                .foregroundStyle(GRUColors.accent)

            Text(searchText.isEmpty ? "Пока тихо" : "Ничего не найдено")
                .font(.headline)
